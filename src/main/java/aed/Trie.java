...
public class Trie{
    ...
    private void obtenerClavesRec(TrieNode nodo, String prefijo, ArrayList<String> claves) { // O(c∈C Σ|c|) || O(mc∈Mc Σ|mc|) 
        ...
        for (int i = 0; i < nodo.hijos.length; i++) {                                     // O(1) porque es un array de 256.
            if (nodo.hijos[i] != null) {                                                     // O(1)                                              
                obtenerClavesRec(nodo.hijos[i], prefijo + ((char) i), claves);               // O(c∈C Σ|c|) || O(mc∈Mc Σ|mc|)  
            }
        }
    }

    private void limpiar(TrieNode nodo) { // O(c∈C Σ|c|) || O(mc∈Mc Σ|mc|) 
        if (nodo.valor == null) {
            nodo = null;
            return;}                                                          // O(1)
        if (nodo.valor != null) {return;}                                       // O(1)   
        for (int i = 0; i < nodo.hijos.length; i++) {                                     // O(1) porque es un array de 256.
            if (nodo.hijos[i] != null) {                                                     // O(1)                                              
                limpiar(nodo.hijos[i]);               // O(c∈C Σ|c|) || O(mc∈Mc Σ|mc|)  
            }
        }
    }
    ...
    *   / 
    *  a
    * eliminar "pata" iria hasta la ultima letra y al volver recursivamente eliminaria el nodo a, t y no haria nada
    * en el nodo de la letra a y p ya que indican que hay otra palabras, en este caso "par" y (otras letras).
